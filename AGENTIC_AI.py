...
with st.sidebar:
	...
	st.caption(f"🔸 Missing: {st.session_state['missing_product']}")
	st.caption(f"🔸 Substitute: {st.session_state['substitute_product']}")
	...
	with log_box.container():
		for msg in st.session_state["call_log"]:
			...
			with st.chat_message(role):
				st.write(msg["text"])
				st.markdown(f'<span style="color: {sent_color}; font-size: 0.85rem; font-weight: 600;">{sent_label} ({msg["sentiment"]:.2f})</span>', unsafe_allow_html=True)
